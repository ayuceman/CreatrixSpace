        ...
        <Card>
          <CardContent className="p-12">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-fg-2">Loading locations and plans...</p>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    return (
      <div className="space-y-8">
        <Card>
          <CardContent className="p-12">
            <div className="flex flex-col items-center justify-center space-y-4">
              <AlertCircle className="h-8 w-8 text-clay-deep" />
        ...
        <CardContent>
          {locations.length === 0 ? (
            <div className="p-8 text-center">
              <MapPin className="h-12 w-12 mx-auto mb-4 text-fg-2 opacity-50" />
              <p className="text-fg-2 mb-2">No locations available</p>
              <p className="text-sm text-fg-2">
                Please add locations in your Supabase database or check your
                connection.
              </p>
            </div>
          ) : (
            ...
            >
              {locations.map((location) => (
                <div
                  key={location.id}
                  className="flex items-center space-x-3 p-4 border rounded-lg hover:bg-bg-band/50 transition-colors"
                >
                  <RadioGroupItem
                    value={location.id}
                    id={location.id}
          ...
          <CardContent>
                ...
                const roomImage =
                  room.imageUrl && room.imageUrl.trim() !== ''
                    ...
                    : ROOM_IMAGE_FALLBACKS[
                        room.slug?.toLowerCase() || room.name.toLowerCase()
                      ]
                return (
                  <button
                    key={room.id}
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? 'border-clay shadow-lg ring-1 ring-primary/30'
                        : 'hover:border-clay/40',
                      isUnavailable
                        ? 'opacity-60 cursor-not-allowed bg-bg-band/40'
                        : ''
                    )}
                  >
                    <div className="aspect-video rounded-lg overflow-hidden bg-bg-band">
                      {roomImage ? (
                        <img
                          src={roomImage}
                          alt={room.name}
                        ...
                        <div className="h-full w-full flex items-center justify-center text-fg-2 text-sm">
                          Image coming soon
                        </div>
                      )}
                    </div>
                    ...
                    <div className="mt-4 flex items-start justify-between gap-2">
                      <div>
                        ...
                        {room.description && (
                          <p className="text-sm text-fg-2 mt-1 line-clamp-2">
                            {room.description}
                          </p>
                        )}
                      </div>
            ...
            {locationRooms.length > 0 && roomStatusMessage && (
              <p className="text-xs text-fg-2 mt-4">
                Room pricing automatically adjusts plan totals.{' '}
                {roomStatusMessage}
              </p>
            )}
          </CardContent>
        ...
        <CardContent>
          {requiresRoomSelection && !bookingData.roomId && (
            <p className="mb-4 text-sm text-amber-600">
              Choose an available room above to unlock plan-specific pricing for
              this location.
            </p>
          )}
          ...
          {plans.length === 0 ? (
            <div className="p-8 text-center">
              <Users className="h-12 w-12 mx-auto mb-4 text-fg-2 opacity-50" />
              <p className="text-fg-2 mb-2">No plans available</p>
              <p className="text-sm text-fg-2">
                Please add plans in your Supabase database or check your
                connection.
              </p>
            </div>
          ) : (
            ...
            >
              {plans.map((plan) => {
                ...
                const isDhobighat =
                  ...
                  (selectedLocation.slug === 'dhobighat-hub' ||
                    selectedLocation.name?.toLowerCase().includes('dhobighat'))
                const isDhobighatRestrictedPlan =
                  isDhobighat &&
                  (plan.type === 'dedicated_desk' ||
                    plan.type === 'private_office')
                const dhobighatStatus = isDhobighatRestrictedPlan
                  ? 'Fully booked for 6 months'
                  : null

                const isAvailable =
                  plan.available !== false && !isDhobighatRestrictedPlan
                const planDisabled =
                  !isAvailable || (requiresRoomSelection && !bookingData.roomId)
                ...
                return (
                  <div
                    ...
                    className={`flex items-center space-x-3 p-4 border rounded-lg transition-colors ${
                      planDisabled
                        ? 'opacity-60 bg-bg-band/30 cursor-not-allowed'
                        : 'hover:bg-bg-band/50'
                    }`}
                  >
                    <RadioGroupItem
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-fg-2">
                            ...
                            {plan.type === 'private_office' &&
                              'Ultimate privacy and productivity'}
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          {plan.type === 'day_pass' && plan.pricing.daily && (
                            <div className="mb-1 flex items-center justify-end gap-2">
                              ...
                              <span className="text-xs text-fg-2 line-through">
                          ...
                          <div className="font-bold text-lg">
                            {primaryPrice
                              ? formatCurrency(primaryPrice, 'NPR')
                              : 'Contact sales'}
                          </div>
                          <div className="text-sm text-fg-2">per {period}</div>
                          {!isDayPass &&
                            locationPricing.weekly &&
                            locationPricing.monthly && (
                              <div className="text-xs text-fg-2">
                                Weekly:{' '}
                                {formatCurrency(locationPricing.weekly, 'NPR')}
                              </div>
                            )}
                          {!isDayPass && locationPricing.annual && (
                            <div className="text-xs text-green-600">
                              Annual:{' '}
                              {formatCurrency(locationPricing.annual, 'NPR')}
                            </div>
                          )}
                        </div>
                      </div>
        ...
        <Card className="bg-clay/5 border-clay/20">
          <CardContent className="p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {selectedRoom && (
                <div className="flex justify-between">
                  <span className="text-fg-2">Room:</span>
              ...
              <div className="flex justify-between">
                <span className="text-fg-2">Price:</span>
                <span className="font-medium">
                  {formatCurrency(
                    selectedPlan?.type === 'day_pass'
                      ? selectedPlanPricing?.daily ||
                          selectedPlan?.pricing.daily ||
                          0
